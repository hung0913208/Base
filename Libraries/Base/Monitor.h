...
class Monitor {
public:
  enum ActTypeE { EAppend = 0, EModify = 1, EFind = 2, ERemove = 3 };

  enum TypeE { EIOSync = 0, EPipe = 1, ETimer = 2 };
  ...
  ErrorCodeE Scan(Auto fd, Int mode,
  ...
  /* @NOTE: access the head monitor, they are defined as a single tone and
   * should be the latest remover */
  Monitor *&Head();
...
protected:
  ...
  /* @NOTE: these virtual method are used to register a triggering with
   * specific events */
  ...
  /* @NOTE: this template is used to convert an object to a specific pointer and
   * this pointer is collected by a linked list and will be remove if the whole
   * Monitor object is deleted */
  template <typename T> T *Allocate(T data) {
    T *result = new T(data);

    if (_Garbage.Add(result) <= 0) {
      delete result;
      return None;
    }
