...
ErrorCodeE Protect(Void* address, UInt len, Int codes) {
  #define MPROTECT_CODES  PROT_NONE | PROT_READ | PROT_WRITE | PROT_EXEC  \
                                    | PROT_GROWSUP | PROT_GROWSDOWN

  auto pagesize = sysconf(_SC_PAGE_SIZE);
  auto pageno = (Long)address/pagesize;

  switch(mprotect((Void*) (pageno * pagesize), (Long)pageno + len, codes)) {
    case EACCES:
    ...
    case EINVAL:
      if ((PROT_GROWSUP & codes) && (PROT_GROWSDOWN & codes)) {
        ...
        return BadAccess("address is not a valid pointer or not a multiple"
                         " of the system page size").code();
      }

    case ENOMEM:
      return DrainMem("Kernel can't allocate or you're changing a part which"
                      " has already changed with the same codes").code();
