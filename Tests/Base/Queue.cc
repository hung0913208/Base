...
TEST(Queue, ThreadsWithoutSleep) {
  Base::Queue<Int> queue{};
  Function<void()> perform = [&]() {
...
TEST(Queue, Bandwidth) {
  Base::Queue<Int> queue{};
  Function<void()> perform = [&]() {
    Base::Thread ithreads[MAX_THREADS];
    Int counter = 0;

    for (auto i = 0; i < MAX_THREADS; ++i) {
      ithreads[i].Start([&counter, &queue]() {
        INC(&counter); 
        queue.Put(RValue(counter));
      });
    }
    ...
    for (auto i = 0; i < MAX_THREADS; ++i) {
      ...
      EXPECT_TRUE(queue.Done(queue.Get(value)));
    }
  };
}
